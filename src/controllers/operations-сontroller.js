...
const processUserData = (userData) => {
  return simulateAsyncOperation({ processedData: `${userData.data} Processing complete!` });
};
...
const triggerOperations = async (req, res) => {
  try {
    ...
    if (error.isCanceled) {
      res.status(500).json({ error: 'Operation canceled', message: error.message });
    } else {
      res.status(500).json({ error: 'Error during execution', message: error.message });
    }
  }
};
...
module.exports = { simulateAsyncOperation, triggerOperations, cancelOperations };
